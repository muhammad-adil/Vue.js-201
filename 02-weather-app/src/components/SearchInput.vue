  ...
  searchTerm.timeout = setTimeout(async () => {
    if (searchTerm.query !== "") {
      const res = await fetch(
        `http://api.weatherapi.com/v1/search.json?key=5b3ed35c01eb47bf83f224919241507&q=${searchTerm.query}`
      );
...
const getWeather = async (id) => {
  const res = await fetch(
    `http://api.weatherapi.com/v1/forecast.json?key=5b3ed35c01eb47bf83f224919241507&q=id:${id}&days=3&aqi=no&alerts=no`
  );
